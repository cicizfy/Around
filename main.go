...
func main() {
	...
	r := mux.NewRouter()
	//OPTIONS prevent CORS(allows restricted resources on a web page to be requested from
...
func handlerPost(w http.ResponseWriter, r *http.Request) {
	...
	//annote image with vision api
	if p.Type == "image" {
		//internal ip address(URI)
	...
	//Save post to ES. POST_INDEX== which db?
...
func saveToGCS(r io.Reader, objectName string) (*storage.ObjectAttrs, error) {
	...
	//Access control
	// Default: only owener(service account)can read files(private).
	// Need to change to all users can read files(public).
...
func saveToES(i interface{}, index string, id string) error {
	...
	//Index == insert
	_, err = client.Index().
		Index(index). //not same as above Index
		Id(id).
		BodyJson(i).
		Do(context.Background()) //do == excute
